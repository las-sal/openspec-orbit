...
- **New command** `/opsx:review <name> [--as proposal|system]` — unified editorial review command with two modes. `--as proposal` (pre-apply) runs 9 passes over change artifacts; `--as system` (post-apply) wraps upstream `verify-change` as Pass 0 and adds 6 system-wide passes (baseline compliance, cohesion, surface walk, perspectives, critical paths, drift). Mode inferred from `tasks.md` state when omitted. Findings roll into a 3-dimension scorecard with CRITICAL/WARNING/SUGGESTION severities.
