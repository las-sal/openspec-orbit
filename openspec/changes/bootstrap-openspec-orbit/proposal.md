...
- **New command** `/opsx:review-external` — packages a review request for an external AI (codex, fresh Claude, etc.); emits a self-contained prompt to chat; defines the file-based findings format that closes the cross-AI loop without copy-paste-per-finding.
