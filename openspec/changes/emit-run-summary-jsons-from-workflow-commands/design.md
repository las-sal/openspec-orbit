...
- **Workflow commands emit nothing.** `/opsx:explore`, `/opsx:propose` (and variants `/opsx:new`, `/opsx:continue`, `/opsx:ff-change`), `/opsx:apply`, `/opsx:verify` all produce artifacts but no run-summary JSON. State is recoverable only indirectly, via artifact presence on disk.
...
- Harmonizing existing emit-producing capabilities (`orbit-review`, `orbit-address-reviews`, etc.) with the new spine. Existing emits already conform de facto; formal cross-references are a follow-up.
...
`/opsx:new`, `/opsx:continue`, `/opsx:ff-change` are propose-shaped (produce the canonical artifact set), but each emits with its own command-name prefix: `new-<TS>.json`, `continue-<TS>.json`, `ff-change-<TS>.json`. Same shape, distinct origins.
...
| `propose`/`new`/`ff-change` | always | `/opsx:review <name>` (per [#9](https://github.com/las-sal/openspec-orbit/issues/9)) |
...
1. Land this change (orbit's emit behavior for the 9 new commands)
   ...
   - Includes updating the 3 existing per-skill schema reference files (`.claude/skills/openspec-{review,audit-drift,address-reviews}/references/run-summary-schema.md`) to acknowledge inheritance from orbit-conventions's universal spine — doc-only updates, no behavior change (tracked in tasks.md group 1.2)
