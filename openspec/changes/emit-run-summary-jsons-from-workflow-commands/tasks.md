...
- [ ] 9.5 (Per **T1** resolution) Post-task-1.4 emit-shape verification: run `/opsx:review`, `/opsx:address-reviews`, `/opsx:archive` (existing emitters) and verify their emit JSONs now include the INTENDED spine additions from task 1.4 — `kind: "editorial"` for review and address-reviews; `kind: "lifecycle"` + `final_assessment` + `next_recommended` for archive. Separately verify NO fields beyond the task-1.4 additions were removed, renamed, or repurposed in those emit JSONs (guard against unintended regressions in other fields). The two checks together replace the prior "verify shapes are unchanged" framing, which was wrong because shapes ARE intentionally changed by task 1.4.
