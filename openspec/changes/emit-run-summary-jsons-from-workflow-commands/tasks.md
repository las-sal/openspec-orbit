...
- [ ] 1.4 (Per **S1** + **F1** resolutions) Update existing emit-producing skill SKILL.md instructions (or `## Orbit additions` sections) to align their actual emit JSONs with the orbit-conventions universal spine. Per-skill scope (each verified against existing archived JSONs):
...
- [ ] 1.5 (Per **F1** verification gate) Add a verify-time check in task 9.5: after the change lands, the next `/opsx:archive` invocation's emit MUST be spot-checked to confirm `kind`, `final_assessment`, and `next_recommended` are all present per the universal spine. If absent, task 1.4 wasn't completed correctly.
