...
- **Drift between SKILL prose and actual behavior** if auto-discovery surfaces edge cases not captured in spec scenarios. Mitigation: 9 net-new scenarios cover the happy path (1) + 7 edge cases (markers-found-precedence / no-candidates / explicit `--from-file` override / multiple candidates with recency tie-break / standard ingest lifecycle / `--mark` no-longer-prerequisite / archive-location respected) + 1 audit-trail-evidence scenario codifying resolution-log fields for the discovery decision.
