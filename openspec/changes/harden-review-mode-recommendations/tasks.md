...
- [ ] 1.1 Update `.claude/skills/openspec-review/SKILL.md`'s `### 9. Emit the final assessment` section. Replace the system-mode rows in the stock-phrasings table with the 4 new rows from the MODIFIED `Final assessment phrasings depend on mode` requirement: system-mode "no CRITICAL, no prior external" + "no CRITICAL, external converged clean" + "no CRITICAL, external stale" cases, each split into "all clear" and "Only WARNING/SUGGESTION" sub-cases per the spec. Add the iteration-aware logic prose: how to detect `external-system-*.md` presence + how to compare timestamps against the most recent `review-system-*.json`.
