...
The `/opsx:review-external` command — packages a review request for an external AI (codex, fresh Claude, GPT, etc.) to perform a second-opinion review. Writes a self-contained markdown prompt to `.orbit-runs/external-prompt-<as>-<TS>.md` and emits a short chat invocation snippet. Does NOT run the review or ingest findings; ingestion is `/opsx:address-reviews --from-file`. Includes mode inference, iteration counting per mode, cycle-context population from prior runs, and a recommended-session note tailored to iteration history.
